...
PICOCOMTERMINAL=T2 #terminal name which will open picocom for zedboard
...
screen -S $PICOCOMTERMINAL -X stuff 'echo insert sd card into zedboard and type password for picocom'`echo -ne '\015'`
screen -S $PICOCOMTERMINAL -X stuff 'sudo picocom -b 115200 /dev/ttyACM0'`echo -ne '\015'`
screen -S $PICOCOMTERMINAL -X stuff '2'`echo -ne '\015'`
sleep 1

#TODO if zedboard needs to be restarted, restarted
